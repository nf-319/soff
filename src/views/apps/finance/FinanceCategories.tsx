    ...
    <Grid container spacing={6} mb={10}>
      {categryData.map((_: any) => (
        <Grid
          ...
          lg={2}
          key={String(_.id)}
          onMouseEnter={() => setIsHover(_.id)}
          onMouseLeave={() => setIsHover(null)}
        >
          <Box
            onClick={() => (_.is_advanced ? push(`/finance/advance`) : push(`/finance/costs/${_.id}`))}
            sx={{ cursor: 'pointer', position: 'relative' }}
          >
            <CardFinanceCategory
              title={_.name}
              stats={_.total_expense}
              icon={<IconifyIcon fontSize={'3rem'} icon={''} />}
              color={'warning'}
            />
            {isHover === _.id && _.is_delete && (
              <IconButton
                onClick={e => (e.stopPropagation(), setDeleteCategory(_.id))}
                aria-label='delete'
                size='small'
                style={{ position: 'absolute', zIndex: 999, bottom: '2px', right: 0, cursor: 'pointer' }}
              >
      ...
      <Dialog open={Boolean(deleteCategory)} onClose={() => setDeleteCategory(null)}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', minWidth: '300px', justifyContent: 'space-between' }}>
          <IconButton onClick={() => setDeleteCategory(null)}>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: 4, mt: 10 }}>
            <LoadingButton onClick={() => setDeleteCategory(null)} variant='outlined'>
              {t('Bekor qilish')}
            </LoadingButton>

            <LoadingButton loading={loading} onClick={() => confirmDeleteCategory()} color='error' variant='contained'>
              {t("O'chirish")}
            </LoadingButton>
          </Box>
        </DialogContent>
