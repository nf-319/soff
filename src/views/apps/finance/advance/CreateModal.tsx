...
export default function CreateModal() {
  ...
  const { isOpenCreateModal, formikState, employee, queryParams } = useAppSelector(state => state.advanceSlice)
  ...
  const [month] = useState<string>(allNumbersParams.date_month)
  const { t } = useTranslation()
  const [employees, setEmployes] = useState<EmployeeItemType[] | null>(null)

  async function getEmployee() {
    await api
      .get(`${ceoConfigs.employee_checklist}?type=employee`)
      .then(res => {
