...
import {
  ...
  setOpenEdit,
  updateAdvance
} from 'src/store/apps/finance/advanceSlice'
import { IAdvanceFormState } from 'src/types/apps/finance'
import * as Yup from 'yup'

export default function EditModal() {
  const [loading, setLoading] = useState<boolean>(false)
  const { isEditId, advanceList, formikState, employee, queryParams } = useAppSelector(state => state.advanceSlice)
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: Partial<IAdvanceFormState>) => {
      ...
      const response = await dispatch(
        updateAdvance({ ...values, amount: revereAmount(values.amount || ''), id: Number(isEditId) })
      )
      if (response.meta.requestStatus == 'rejected') {
        ...
        await dispatch(getAdvanceList(String(queryString)))
      }
      setLoading(false)
    }
  })
