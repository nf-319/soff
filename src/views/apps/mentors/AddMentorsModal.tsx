...
export default function AddMentorsModal() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: CreateTeacherDto) => {
      ...
      for (const [key, value] of Object.entries({ ...values, amount: revereAmount(String(values.amount)) })) {
        if (key === 'phone') {
          newValues.append(key, reversePhone(String(value)))
        } else {
          newValues.append(key, value as string)
        }
      }
